...
def sogou_search(user, request):
    ...
    if query != '':
        ...
        results = None
        try:
            results = SogouResults.objects.get(query=query)
        except DoesNotExist:
            # if it has not, crawl results using search api
            results = utils.crawl_sogou(query)
...
def baidu_search(user, request):
    ...
    if query != '':
        ...
        results = None
        try:
            results = BaiduResults.objects.get(query=query)
        except DoesNotExist:
            # if it has not, crawl results using search api
            results = utils.crawl_baidu(query)

        start_at = 0
        end_at = min(len(results.results), start_at + 20)
